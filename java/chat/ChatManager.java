...
public class ChatManager {
   ...
   public DeviceData getReceivingRadio(int var1) {
      if (this.player.getOnlineID() == -1) {
         return null;
      } else {
         VoiceManagerData var2 = VoiceManagerData.get(this.player.getOnlineID());
         synchronized(var2.radioData) {
            return (DeviceData)var2.radioData.stream().filter((var1x) -> {
               return var1x.isReceivingAvailable(var1);
