...
public class AddXp implements INetworkPacket {
   ...
   public boolean validate(UdpConnection var1) {
      if (var1.accessLevel != 1 && var1.accessLevel != 2) {
         ...
         if (ServerOptions.instance.AntiCheatProtectionType14.getValue() && PacketValidator.doKickUser(var1, this.getClass().getSimpleName(), "UI_ValidationFailed_Type14")) {
         ...
         if (ServerOptions.instance.AntiCheatProtectionType15.getValue() && PacketValidator.doKickUser(var1, this.getClass().getSimpleName(), "UI_ValidationFailed_Type15")) {
         ...
         if ((double)this.amount > 1000.0D * SandboxOptions.instance.XpMultiplier.getValue() * ServerOptions.instance.AntiCheatProtectionType15ThresholdMultiplier.getValue() / 2.0D) {
            LoggerManager.getLogger("user").write(String.format("Warning: player=\"%s\" type=\"%s\" issuer=\"%s\"", var1.username, "UI_ValidationFailed_Type15", this.getClass().getSimpleName()));
         }
