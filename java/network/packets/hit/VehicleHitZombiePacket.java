...
public class VehicleHitZombiePacket extends VehicleHitPacket implements INetworkPacket {
   ...
   public boolean validate(UdpConnection var1) {
      if (!PacketValidator.checkType2(var1, this.vehicleHit, VehicleHitZombiePacket.class.getSimpleName())) {
         return false;
      } else if (!PacketValidator.checkType8(var1, this.wielder, this.target, VehicleHitZombiePacket.class.getSimpleName())) {
         return false;
      } else {
         return PacketValidator.checkType4(var1, this.vehicleHit, VehicleHitZombiePacket.class.getSimpleName());
      }
   }
}
