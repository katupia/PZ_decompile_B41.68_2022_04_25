...
public class ZombieHitPlayerPacket extends HitCharacterPacket implements INetworkPacket {
   ...
   public boolean validate(UdpConnection var1) {
      if (!PacketValidator.checkType8(var1, this.wielder, this.target, ZombieHitPlayerPacket.class.getSimpleName())) {
         return false;
      } else if (!PacketValidator.checkType5(var1, this.wielder, ZombieHitPlayerPacket.class.getSimpleName())) {
         return false;
      } else {
         return PacketValidator.checkType6(var1, this.target, ZombieHitPlayerPacket.class.getSimpleName());
      }
   }
}
