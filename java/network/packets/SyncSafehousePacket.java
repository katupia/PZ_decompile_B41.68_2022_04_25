   ...
   public boolean validate(UdpConnection var1) {
      ...
      if (this.safehouse == null) {
         if (var1.accessLevel == 1 && SafeHouse.hasSafehouse(this.ownerUsername) != null) {
            if (ServerOptions.instance.AntiCheatProtectionType19.getValue() && PacketValidator.doKickUser(var1, this.getClass().getSimpleName(), "UI_ValidationFailed_Type19")) {
               LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\" issuer=\"%s\" description=\"%s\"", var1.username, "UI_ValidationFailed_Type19", this.getClass().getSimpleName(), this.getDescription()));
            }
            ...
            if (var1.accessLevel != 1 || !((double)this.h > var3) && !((double)this.w > var3)) {
               ...
               if (ServerOptions.instance.AntiCheatProtectionType20.getValue() && PacketValidator.doKickUser(var1, this.getClass().getSimpleName(), "UI_ValidationFailed_Type20")) {
                  LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\" issuer=\"%s\" description=\"%s\"", var1.username, "UI_ValidationFailed_Type20", this.getClass().getSimpleName(), this.getDescription()));
               }
         ...
         return PacketValidator.checkType7(var1, this.safehouse.getOwner(), SyncSafehousePacket.class.getSimpleName());
      }
   }
