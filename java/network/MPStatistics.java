   ...
   private static void limitPing() {
      ...
      while(var1.hasNext()) {
         ...
         if (doKick(var2, var3)) {
            ...
            LoggerManager.getLogger("kick").write(String.format("Kick: player=\"%s\" type=\"%s\"", var2.username, "UI_OnConnectFailed_Ping"));
            LoggerManager.getLogger("kick").write(String.format("Ping: limit=%d/%d average-%d=%d", var0, pingLimitCount, pingIntervalCount, var3));
            LoggerManager.getLogger("kick").write(String.format("Ping: last-%d: %s", 120L, var2.pingHistory.stream().map(Object::toString).collect(Collectors.joining(", "))));
         }
