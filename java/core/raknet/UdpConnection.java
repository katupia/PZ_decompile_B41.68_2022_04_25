...
public class UdpConnection {
   ...
   public ZNetSessionState netSessionState;
   ...
   public void setFullyConnected() {
      ...
      this.connectionTimestamp = System.currentTimeMillis();
      ConnectionManager.log("Player Fully Connected", this);
   }
   ...
   public ZNetSessionState getP2PSessionState() {
      try {
         this.netSessionState = this.engine.peer.GetP2PSessionState(this.connectedGUID);
      } catch (Exception var5) {
         this.netSessionState = null;
      } finally {
         return this.netSessionState;
      }
   }

   public String getDescription() {
      long var10000 = this.connectedGUID;
      return "connection guid=\"" + var10000 + "\" username=\"" + this.username + "\" access=\"" + this.accessLevel + "\" ip=\"" + this.ip + "\" steamID=\"" + this.steamID + "\" players=\"" + Arrays.stream(this.players).filter(Objects::nonNull).count() + "\" preferred=\"" + this.preferredInQueue + "\" wait=\"" + LoginQueue.isInTheQueue(this) + "\" queued=\"" + this.wasInLoadingQueue + "\" full=\"" + this.bFullyConnected + "\" mtu=\"" + this.getMTUSize() + "\"";
   }
   ...
   public String getPlayerDescription() {
      StringBuilder var1 = new StringBuilder("players");

      for(short var2 = 0; var2 < 4; ++var2) {
         var1.append(" ").append(var2).append("=[").append(this.playerIDs[var2]);
         if (this.players[var2] != null) {
            var1.append(" ").append(this.players[var2].getOnlineID()).append(" ").append(this.players[var2].getUsername());
         }
